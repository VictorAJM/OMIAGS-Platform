  ...
  async function loadEnrollmentStatus() {
    try {
      ...
      if (res.ok) {
        ...
        completedLessonIds = (data.completedLessons || []).map((id) =>
          id.toString(),
        );
      }
    } catch (err) {
      console.warn(
        "No se pudo cargar el estado inicial de la inscripción",
        err,
      );
    }
  }
  ...
  async function toggleCompleted(lesson) {
    ...
    try {
      ...
      if (!newStatus) {
        ...
        completedLessonIds = completedLessonIds.filter(
          (id) => id !== lesson._id,
        );
      }
...
<div class="course-page">
  <button class="back-btn" on:click={goBack}> ← Volver a mis cursos </button>
      ...
      <div class="lessons-list">
        {#each lessons as lesson, i}
          ...
          <div class="lesson-card {isCompleted ? 'is-completed' : ''}">
            <div
              class="lesson-header"
              on:click={() => toggleLesson(lesson._id)}
            >
