<script>
  ...
  import NavBar from "$lib/components/NavBar.svelte"; // Asumiendo que tienes este componente
  ...
  async function loadContent() {
    try {
      const res = await fetch(`http://localhost:5000/api/lessons/content/${contentId}`);
  ...
  function goBack() {
    // Intenta volver al curso usando el ID que nos dio el backend, si no, al home
