<script lang="ts">
  ...
  async function createCourse(e: Event) {
    // e.preventDefault() handled by on:submit|preventDefault in markup
    loading = true;
    msg = "";
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Error reading enrollments");
      console.log("Enrollments check:", data);
    } catch (err: any) {
  ...
  <div class="modal" on:click|stopPropagation in:scale={{ start: 0.96, duration: 200 }}>
    ...
    <div class="modal-header">
        <div class="header-content">
            ...
            <p class="modal-subtitle">Configura los detalles básicos y el acceso inicial.</p>
        </div>
    ...
    <div class="modal-body-scroll">
        ...
        <form id="create-course-form" on:submit|preventDefault={createCourse}>
            <div class="main-fields">
                <div class="form-row">
                    ...
                    <div class="form-group flex-1">
                        <label for="category">Nivel</label>
            ...
            <div class="students-section">
                <div class="section-header">
                    <label for="student-input">Inscribir Estudiantes</label>
                    <span class="badge">{students.length} listos</span>
                </div>
                <p class="help-text">Puedes agregar correos electrónicos ahora o hacerlo más tarde.</p>
                ...
                <div class="input-group">
                    ...
                    <button type="button" class="btn-add" on:click={addStudent}>Agregar</button>
                </div>
...
<style>
  ...
  .modal {
    background: #ffffff; border-radius: 16px; width: 100%; max-width: 550px; height: auto; max-height: 90vh;
  ...
  .form-row { display: flex; gap: 1rem; margin-bottom: 0; }
  .flex-2 { flex: 2; }
  .flex-1 { flex: 1; }
  ...
  .badge { background: #e2e8f0; color: #475569; font-size: 0.75rem; padding: 0.15rem 0.6rem; border-radius: 12px; font-weight: 600; }
  .help-text { font-size: 0.8rem; color: #64748b; margin: 0 0 1rem 0; }

  .input-group { display: flex; gap: 0.5rem; margin-bottom: 1rem; }
  .input-wrapper { position: relative; flex: 1; }
  .input-icon { position: absolute; left: 10px; top: 50%; transform: translateY(-50%); font-size: 1rem; pointer-events: none; }
  .input-wrapper input { padding-left: 2.2rem; }

  .btn-add {
    background: #3b82f6; color: white; border: none; padding: 0 1.2rem; border-radius: 8px;
  ...
  .student-list-container {
    min-height: 100px; max-height: 150px; overflow-y: auto; background: #ffffff;
    border: 1px solid #e2e8f0; border-radius: 8px; padding: 0.75rem;
  }
  .tags-grid { display: flex; flex-wrap: wrap; gap: 0.5rem; }

  .student-tag {
    display: inline-flex; align-items: center; background: #eff6ff; color: #1e40af;
    padding: 0.3rem 0.6rem; border-radius: 6px; border: 1px solid #bfdbfe; font-size: 0.85rem; font-weight: 500;
  }
  .tag-remove {
    background: transparent; border: none; color: #60a5fa; margin-left: 0.5rem; font-size: 1.1rem;
    line-height: 1; cursor: pointer; padding: 0 2px; display: flex; align-items: center;
  }
  .tag-remove:hover { color: #ef4444; }

  .empty-list {
    display: flex; flex-direction: column; align-items: center; justify-content: center;
    height: 100%; color: #94a3b8; font-size: 0.9rem; gap: 0.5rem; padding: 1rem 0;
  }
  .empty-list span { font-size: 1.5rem; opacity: 0.5; }
