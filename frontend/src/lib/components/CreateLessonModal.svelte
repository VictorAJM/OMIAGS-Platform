<script lang="ts">
  ...
  function removeFile(index: number) {
    if (contents[index]) {
      contents[index].url = "";
      // Optional: clear title if it was auto-filled from filename
  ...
  function handleQuizCreated(event: CustomEvent) {
    ...
    if (activeContentIndex !== null && contents[activeContentIndex]) {
      ...
      window.open(`/editor/quizzes/${newQuiz._id}`, "_blank");
    }
  ...
  async function handleSubmit() {
    ...
    for (const c of contents) {
      ...
      if (c.type === "quiz" && !c.quizId) {
        error = "Todos los quizzes deben estar vinculados.";
        return;
      }
    }
    ...
    try {
      ...
      // Update quizzes with the new lessonId
      for (const c of contents) {
        if (c.type === "quiz" && c.quizId) {
          try {
          ...
          {#if contents.length === 0}
            <div class="empty-state">
              ...
              <button type="button" class="btn-link" on:click={addContent}
                >Agregar primer material</button
              >
            </div>
          {:else}
                          ...
                          {#if content.quizId}
                            <div class="quiz-linked-card">
                              <div class="quiz-details">
                                ...
                                <div class="meta">
                                  <span class="id">ID: {content.quizId}</span>
                                </div>
                              </div>
                        ...
                        {#if content.url}
                          <div class="file-preview-card">
                            ...
                            <button
                              ...
                              on:click={() => removeFile(i)}>Cambiar</button
                            >
                          </div>
                        {:else}
