...
router.get("/my-students", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const myCourses = await Course.find({ owner: instructorId }).select(
      "_id title",
    );
    ...
    const quizzes = await Quiz.find({ lessonId: { $in: lessonIds } }).select(
      "_id lessonId maxScore",
    );
    ...
    quizzes.forEach((quiz) => {
      const lesson = lessons.find(
        (l) => l._id.toString() === quiz.lessonId.toString(),
      );
      if (lesson) {
    ...
    enrollments.forEach((enrollment) => {
      ...
      const studentCourseAttempts = attempts.filter(
        (a) =>
      ...
      if (studentCourseAttempts.length > 0) {
        ...
        studentCourseAttempts.forEach((att) => {
          ...
          const quizRef = quizzes.find(
            (q) => q._id.toString() === att.quizId.toString(),
          );
